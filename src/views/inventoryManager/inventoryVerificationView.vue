<template>
    ...
    <div class="query-container">
      ...
      <div class="query-item">
        ...
        <el-select v-model="query.businessType" clearable placeholder="请选择" style="width: 150px">
          <el-option
            v-for="item in OUTBOUND_BUSINESS_TYPES"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
  ...
  <el-dialog v-model="viewVisible" title="查看出库单据" width="980px">
    <div v-if="currentHeader" class="dialog-content">
      ...
      >
        ...
        <el-table-column
          v-if="currentHeader.auditStatus !== 2"
  ...
  <el-dialog v-model="auditVisible" title="审核出库单据" width="980px">
    <div v-if="currentHeader" class="dialog-content">
...
const isTransferDocument = computed(() => currentHeader.value?.businessType === 6)
...
function canAuditRow(row: OutboundAuditListItem) {
  return isWarehouseSpecialist.value && row.auditStatus === 1
}

async function openView(row: OutboundAuditListItem) {
  await fetchDocument(row.billCode)
  viewVisible.value = true
}

async function openAudit(row: OutboundAuditListItem) {
  await fetchDocument(row.billCode)
  rejectReason.value = ''
