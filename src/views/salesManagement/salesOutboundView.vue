<template>
  <div class="query-block">
    <div class="query-container">
      <QueryInput v-model="query.outboundCode" placeholder="请输入单据编号查询" style="width: 220px" />
  ...
  <SalesOutboundDialog
    v-model="editVisible"
    mode="edit"
    :outbound-code="editingCode"
    :initial-header="editingHeader"
  ...
  <SalesOutboundDialog
    v-model="viewVisible"
    mode="view"
    :outbound-code="viewingCode"
    :initial-header="viewingHeader"
  />
</template>
...
import type { SalesOutboundDto, SalesOutboundVO } from '@/type/salesOutbound'
import { useSalesOutbound } from '@/hooks/SalesOutbound/useSalesOutbound'
import SalesOutboundDialog from './salesOutboundDialog.vue'
...
const addDialogRef = ref<InstanceType<typeof SalesOutboundDialog> | null>(null)
const addVisible = ref(false)
const editVisible = ref(false)
const viewVisible = ref(false)
const editingCode = ref('')
const editingHeader = ref<OutboundHeader | null>(null)
const viewingCode = ref('')
const viewingHeader = ref<OutboundHeader | null>(null)
...
const tableColumns = computed<TableColumn[]>(() => [
  { prop: 'outboundCode', label: '销售出库单号', minWidth: 190 },
  ...
  { prop: 'productKindAmount', label: '商品种类数', width: 110 },
  { prop: 'settlementTotalAmount', label: '销售结算总金额', minWidth: 140, formatter: (_r, _c, v) => formatMoney(v) },
...
function canEditRow(row: SalesOutboundVO) {
...
async function handleAddSubmit(dto: SalesOutboundDto, opts?: { saveAndNew?: boolean }) {
  if (!userName.value) {
    ElMessage.warning('当前登录用户异常，请重新登录。')
    return
  }
  await createSalesOutbound(dto)
...
async function handleEditSubmit(dto: SalesOutboundDto) {
  await editSalesOutbound(dto)
  ElMessage.success('修改成功')
  editVisible.value = false
}

async function handleEditCommit(outboundCode: string) {
  try {
    await ElMessageBox.confirm('请确认是否提交当前记录？', '提交销售出库单', {
  ...
  await submitSalesOutbound(outboundCode)
  ElMessage.success('提交成功')
  editVisible.value = false
}

function onView(row: SalesOutboundVO) {
  viewingCode.value = row.outboundCode
  viewingHeader.value = {
...
function onEdit(row: SalesOutboundVO) {
  editingCode.value = row.outboundCode
  editingHeader.value = {
    warehouseCode: row.warehouseCode,
    customerCode: row.customerCode,
    warehouseName: row.warehouseName,
...
async function onSubmit(row: SalesOutboundVO) {
  try {
    await ElMessageBox.confirm('请确认是否提交当前记录？', '提交销售出库单', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
  ...
  await submitSalesOutbound(row.outboundCode)
  ElMessage.success('提交成功')
}

async function onDelete(row: SalesOutboundVO) {
  try {
    await ElMessageBox.confirm('请确认是否删除当前记录？', '删除销售出库单', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
  ...
  await deleteSalesOutbound(row.outboundCode)
