<template>
  <div class="query-block">
    <div class="query-container">
      <QueryInput v-model="query.returnCode" placeholder="请输入采购退货单号查询" style="width: 220px" />
      <QueryInput
        v-model="query.purchaseReceiveCode"
        placeholder="请输入关联采购入库单号查询"
        style="width: 240px"
      />
      <QueryInput v-model="query.warehouseKeyword" placeholder="请输入仓库编号/名称查询" style="width: 220px" />
      <QueryInput v-model="query.supplierKeyword" placeholder="请输入供应商编号/名称查询" style="width: 220px" />
    </div>
    <div class="query-container">
  ...
  <PurchaseReturnDialog v-model="addVisible" mode="add" @submit="handleAddSubmit" />
  ...
  />
  <PurchaseReturnDialog v-model="viewVisible" mode="view" :return-code="viewingCode" />
</template>

<script setup lang="ts">
/**
 * 采购退货：单据提交后由仓库侧出库审核等环节处理（见库存管理），本页不提供审核按钮。
 */
...
import type { PurchaseReturnDto, PurchaseReturnVO } from '@/type/purchaseReturn'
import { usePurchaseReturn } from '@/hooks/PurchaseReturn/usePurchaseReturn'
import PurchaseReturnDialog from './purchaseReturnDialog.vue'
...
const isPurchaseSpecialist = computed(() => roleCode.value === ROLE_PURCHASE_SPECIALIST)
const canAdd = computed(() => isPurchaseSpecialist.value)
const operationWidth = computed(() => (isPurchaseSpecialist.value ? 280 : 100))
...
const tableColumns = computed<TableColumn[]>(() => [
  { prop: 'returnCode', label: '采购退货单号', minWidth: 190 },
  ...
  { prop: 'returnAmount', label: '采退总金额', minWidth: 140, formatter: (_r, _c, v) => formatMoney(v) },
  { prop: 'purchaseReceiveCode', label: '关联采购入库单号', minWidth: 190 },
...
function applyIdentityQuery() {
  if (isPurchaseSpecialist.value) {
    query.value.initiator = userName.value
  }
}

function canEditRow(row: PurchaseReturnVO) {
  return isPurchaseSpecialist.value && (row.auditStatus === 0 || row.auditStatus === 3)
}
...
async function handleAddSubmit(dto: PurchaseReturnDto) {
  if (!userName.value) {
    ElMessage.warning('当前登录用户异常，请重新登录。')
    return
  }
  await createPurchaseReturn(dto)
  ElMessage.success('新增成功')
  addVisible.value = false
}

async function handleEditSubmit(dto: PurchaseReturnDto) {
  await editPurchaseReturn(dto)
  ElMessage.success('修改成功')
  editVisible.value = false
}

function onView(row: PurchaseReturnVO) {
  viewingCode.value = row.returnCode
  viewVisible.value = true
}

function onEdit(row: PurchaseReturnVO) {
  editingCode.value = row.returnCode
  editVisible.value = true
}

async function onSubmit(row: PurchaseReturnVO) {
  try {
    await ElMessageBox.confirm('请确认是否提交当前记录？', '提交采购退货单', {
  ...
  await submitPurchaseReturn(row.returnCode)
  ElMessage.success('提交成功')
}

async function onDelete(row: PurchaseReturnVO) {
  try {
    await ElMessageBox.confirm('请确认是否删除当前记录？', '删除采购退货单', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning',
    })
  ...
  await deletePurchaseReturn(row.returnCode)
