<template>
  <BaseDialog v-model="visible" title="修改供应商" width="700px" @confirm="handleConfirm">
...
import type { SupplierDto, SupplierVO } from "@/type/supplier";
...
watch(
  ...
  async ([open, row]) => {
    ...
    Object.assign(form, {
      ...
      mobile: row.mobile,
      address: row.address,
      checkoutMethod: row.checkoutMethod?.trim() || DEFAULT_CHECKOUT,
      status: row.status,
    });
